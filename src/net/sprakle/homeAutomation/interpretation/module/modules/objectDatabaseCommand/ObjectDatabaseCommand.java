...
public class ObjectDatabaseCommand implements InterpretationModule {
	...
	private NodeType interpretNodeType(Phrase phrase) {
		...
		// binary
		{
			PhraseOutline possibility1 = new PhraseOutline(getName());
			...
			ArrayList<PhraseOutline> binArray = new ArrayList<>();
			binArray.add(possibility1);

			if (phrase.matchOutlines(binArray) != null) {
		...
		// integer
		{
			...
			// tag outlines
			ArrayList<PhraseOutline> setArray = new ArrayList<>();
			setArray.add(poA);
			setArray.add(poB);

			if (phrase.matchOutlines(setArray) != null) {

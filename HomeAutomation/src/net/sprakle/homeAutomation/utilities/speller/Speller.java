...
public class Speller implements SpellCheckListener {
	...
	private final String DICTIONARY_LOCATION;
	...
	public Speller(Logger logger) {
		...
		DICTIONARY_LOCATION = Config.getString("config/spelling/dictionary");
		spellChecker = null;
	...
	public List<String> checkSpelling(String check) {
		...
		if (words.size() > 0) {
			for (Iterator<Word> suggestedWord = words.iterator(); suggestedWord.hasNext();) {
				suggestions.add(suggestedWord.next().toString());
			}
		}
